...
const PostCard: React.FC<PostCardProps> = ({
  ...
  // Monitora visibilidade para Auto-play e Animação do Vinil
  useEffect(() => {
      ...
      ([entry]) => {
        ...
        if (videoRef.current) {
          if (entry.isIntersecting) {
            videoRef.current.play().catch(() => {}); // Catch para evitar erro de política de browser
          } else {
  ...
  return (
    <div className="postcard-container" ref={containerRef}>
      {/* --- MODAIS --- */}
      {showOptionsMenu && (
      ...
      {/* --- CABEÇALHO --- */}
      <div className="postcard-header px-4 md:px-1">
      ...
      {/* --- BLOCO CENTRAL --- */}
      <div className="media-container gap-3 md:gap-5" onClick={handleDoubleTap}>
        <div className="media-wrapper relative">
          ...
          {showHeartOverlay && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-50">
              <Heart
                size={80}
                className="text-white fill-white animate-heart-pop md:w-[100px]"
              />
            </div>
          )}
        </div>
      ...
      <div className="flex w-full mb-4 px-4 md:px-0">
        ...
        <div className="w-[40px] md:w-[46px] shrink-0"></div>
      </div>

      {/* --- FOOTER --- */}
      <div className="postcard-footer px-4 md:px-0">
        <div className="music-info-container flex items-center gap-4">
          ...
          >
            <img
              src={audioCover}
              alt="Capa"
              className={`w-7 h-7 rounded-full object-cover border border-white/10 ${isVisible ? 'animate-spin-slow' : ''}`}
            />
